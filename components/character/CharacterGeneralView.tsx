      ...
      <div className="flex-1 mx-6 pb-8 space-y-8">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-green-100 rounded-xl">
                <Image src={teacherImages.up} alt="positive" width={24} height={24} className="w-6 h-6" />
              </div>
              <div>
                <p className="text-2xl font-bold text-green-600">{data?.good_character_sum_all_class ?? "..."}</p>
                <p className="text-sm font-medium text-gray-600">Positives</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-red-100 rounded-xl">
                <Image src={teacherImages.down} alt="growth" width={24} height={24} className="w-6 h-6" />
              </div>
              <div>
                <p className="text-2xl font-bold text-red-600">{data?.bad_character_sum_all_class ?? "..."}</p>
                <p className="text-sm font-medium text-gray-600">Areas of Growth</p>
              </div>
            </div>
          </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-600 font-medium">Loading character data...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <div className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-green-50 rounded-lg p-3 text-center">
                        <p className="text-2xl font-bold text-green-600">{classItem.good_character}</p>
                        <p className="text-xs text-green-600 font-medium">Positives</p>
                      </div>
                      <div className="bg-red-50 rounded-lg p-3 text-center">
                        <p className="text-2xl font-bold text-red-600">{classItem.bad_character}</p>
                        <p className="text-xs text-red-600 font-medium">Growth Areas</p>
                      </div>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-lg font-bold text-gray-900">
                          {Math.round((classItem.good_character / (classItem.good_character + classItem.bad_character)) * 100) || 0}%
                        </span>
                      </div>
                      <div className="bg-gray-200 rounded-full h-2">
                        <div className="bg-gradient-to-r from-green-500 to-emerald-500 h-2 rounded-full transition-all duration-500" style={{ width: `${Math.round((classItem.good_character / (classItem.good_character + classItem.bad_character)) * 100) || 0}%` }}></div>
                      </div>
                    </div>
                  </div>
                  <div className="border-t border-gray-100 p-4">
                    <div className="flex items-center justify-between text-sm text-gray-600">
                      <span>Click to manage character</span>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200">
                        ...
                        <th className="px-6 py-4 text-center text-sm font-semibold text-green-700">Positives</th>
                        <th className="px-6 py-4 text-center text-sm font-semibold text-red-700">Areas of Growth</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {data?.classes?.map((classItem: any, index: number) => (
                        <tr key={classItem.id || index} className="hover:bg-gradient-to-r hover:from-orange-50/50 hover:to-red-50/50 cursor-pointer transition-all duration-200 hover:shadow-sm" onClick={() => handleClick(classItem)}>
                          ...
                          <td className="px-6 py-4 text-sm text-green-700 text-center font-semibold">{classItem.good_character}</td>
                          <td className="px-6 py-4 text-sm text-red-700 text-center font-semibold">{classItem.bad_character}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
