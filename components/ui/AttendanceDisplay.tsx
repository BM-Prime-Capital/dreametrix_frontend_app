...
export function AttendanceDisplay({ attendance }: AttendanceDisplayProps) {
  return (
    <div className="flex justify-center gap-2">
      {attendance.present >= 0 && (
        <span key="present" className="text-green-600 font-medium">
          ↑ {attendance.present}
        </span>
      )}
      {attendance.absent !== 0 && (
        <span key="absent" className="text-red-600 font-medium">
          ↓ {Math.abs(attendance.absent)}
        </span>
      )}
      {attendance.late >= 0 && (
        <span key="late" className=" text-purple-600 font-medium">
          → {attendance.late}
        </span>
      )}
      {attendance.half_day >= 0 && (
        <span key="half_day" className=" text-orange-400 font-medium">
          ↕ {attendance.half_day}
        </span>
      )}
    </div>
